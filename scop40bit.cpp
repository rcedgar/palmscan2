...
static void GetDomFromDomSlashFam(const string &Label, string &Dom)
	{
...
void SCOP40Bit::ReadDomInfo()
	{
	string Line;
	vector<string> Fields;

	const string DomFamFN = "d:/int/scop40/out/domain_family.tsv";
	const string DomainsFN = "d:/int/scop40/out/algo_domains.txt";
...
void SCOP40Bit::ReadHits_Tsv(const string &Algo)
	...
	const string TsvFN = "d:/data/scop40pdb/alignResults/rawoutput/" + Algo + "aln";
	uint ScoreFieldNr = 2;
	if (Algo == "foldseek")
		ScoreFieldNr = 10;
	FILE *f = OpenStdioFile(TsvFN);
